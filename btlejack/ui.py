...
from .supervisors import ConnectionRecovery, AccessAddressSniffer, ConnectionSniffer, AdvertisementsSniffer, AdvertisementsJammer, SendTestPacket, ReceiveTestPacket
...
class CLISendTestPacket(SendTestPacket):
    ...
    def __init__(self, devices=None, verbose=None, channel=37):
        ...
        self.verbose = True
        self.send_test_packet(channel)

    def send_test_packet(self, channel):
       ...
       # payload = bytes([0xAA, 0xAA, 0xAA, 0xAA, 0xAA,
        ...
        # payload gedreht
        #payload = bytes([channel, 0x0b, 0xdf, 0xcf, 0x64, 0x4d, 0x5c, 0x7e, 0xa2, 0x04, 0x52, 0x04, 0x9c, 0x42,
                        # 0xbf, 0x4b, 0x38, 0x7a, 0xc8, 0xe8, 0x21, 0x62, 0x41,
                        # 0xbe, 0xe5, 0xe6, 0xd2, 0x44, 0x5d, 0x77, 0x5f, 0xd8,
                        # 0x35, 0xd6, 0x6c, 0x36, 0xb0, 0xd5, 0xcc, 0x88, 0x05,
                        # 0xe9, 0x4e, 0x90, 0x20])

        payload = bytes([channel, 0xb0, 0xfd, 0xfc, 0x46, 0xd4, 0xc5, 0xe7, 0x2a, 0x40, 0x25, 0x40, 0xC9,
                         0x24, 0xFB, 0xB4, 0x83, 0xA7, 0x8C, 0x8E, 0x12, 0x26, 0x14, 0xEB, 0x5E,
                         0x6E, 0x2D, 0x44, 0xD5, 0x77, 0xF5, 0x8D, 0x53, 0x6D, 0xC6, 0x63, 0x0B,
                         0x5D, 0xCC, 0x88, 0x50, 0x9E, 0xE4, 0x09, 0x02])

        # print("Sending this", payload)
        super().send_test_packet(payload, channel)


class CLIReceiveTestPacket(ReceiveTestPacket):
    """
    Copied from CLIAdvertisementsSniffer
    """
